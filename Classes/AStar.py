            ...
            for neighbour in current_node.neighbours:
                ...
                if locationAdd == None:  #BUGFIX
                    ...
                    if tentative_g < neighbour.g:
                        ...
                        neighbour.f = neighbour.g + self.graph.heuristic(neighbour.id, goal.id) #dont need to convert into time because its 60kmph means 1kmpMin = 1:1
...
# def As(start, end):
#     start = next((node for node in graph.nodes if node.id == start), None)
#     goal = next((node for node in graph.nodes if node.id == end), None)
#     openList =[]
#     closedList = []
...
#             return reconstruct_path(current_node, start.id, goal.id)

#         closed_list.append(current_node.id)

#         # Iterate over edges and update neighbour
#         for neighbour in current_node.neighbours:
#             if neighbour.id in Graph.closed_list:
#                 continue

#             tentative_g = current_node.g + costCalc(current_node.id , neighbour.id) #have to call the method to find the address

#             if tentative_g < neighbour.g:
#                 neighbour.g = tentative_g
#                 neighbour.f = neighbour.g +heauristic(neighbour.id , goal.id)
#                 neighbour.parent = current_node

#                 if neighbour not in Graph.open_list:
#                     Graph.open_list.append(neighbour)
#     return